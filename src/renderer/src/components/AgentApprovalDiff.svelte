        ...
        {#if change.kind === 'changed'}
          <div class="grid grid-cols-1 items-stretch gap-2">
            <div class="min-w-0 p-2 border-l-2 border-error bg-error/10 rounded-sm">
              ...
              {#if isLong(change.before)}
                <pre class="text-xs whitespace-pre-wrap break-words overflow-auto max-h-40">{formatValue(change.before)}</pre>
              {:else}
                <div class="text-xs break-words">{formatValue(change.before)}</div>
              {/if}
            </div>
            ...
            <div class="min-w-0 p-2 border-l-2 border-success bg-success/10 rounded-sm">
              ...
              {#if isLong(change.after)}
                <pre class="text-xs whitespace-pre-wrap break-words overflow-auto max-h-40">{formatValue(change.after)}</pre>
              {:else}
                <div class="text-xs break-words">{formatValue(change.after)}</div>
              {/if}
            </div>
          </div>
        {:else}
          <div class="p-2 border-l-2 {change.kind === 'added' ? 'border-success bg-success/10' : 'border-error bg-error/10'} rounded-sm">
            ...
            {#if isLong(change.kind === 'added' ? change.after : change.before)}
              <pre class="text-xs whitespace-pre-wrap break-words overflow-auto max-h-40">{formatValue(change.kind === 'added' ? change.after : change.before)}</pre>
            {:else}
              <div class="text-xs break-words">{formatValue(change.kind === 'added' ? change.after : change.before)}</div>
            {/if}
          </div>
        {/if}
      </div>
