<script lang="ts">
  ...
  function updateHighlighting() {
    ...
    highlightElement.innerHTML = highlightCode(value)
  }
  ...
  function highlightCode(code: string): string {
    ...
    // Use function replacer to avoid $1 backreference issues
    html = html.replace(
  ...
  function highlightJavaScript(code: string): string {
    ...
    // Highlight BCFD variables inside eval blocks: $name
    result = result.replace(/\$([a-zA-Z_][a-zA-Z0-9_]*)/g, (_, name) =>
      addPlaceholder(`<span class="bcfd-variable">$${name}</span>`)
    )
